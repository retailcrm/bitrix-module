...
* Переработан генератор ICML каталога:
    - запись осуществляется через XMLwriter
    - при установке модуля выгрузка по умолчанию теперь не происходит
    - код перенесен в директорию lib и переработан
* Доработан механизм обмена заказами: 
  при указании произвольной цены (CUSTOM_PRICE) в создаваемом заказе,
  скидки больше не учитываются
