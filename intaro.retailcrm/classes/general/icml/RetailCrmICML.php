...
class RetailCrmICML
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            RCrmActions::eventLog('RetailCrmICML', 'highloadblock', 'module not found');
        }
    }
    ...
    public function Load(): bool
    {
        ...
        $defaultSite           = CSite::GetList($byLink, $orderLink, ['DEF' => 'Y'])->Fetch();
    ...
    protected function addManufacturer($propProduct, $arOffer, string $key, string $offer): array
    {
        if ($propProduct !== "" && $arOffer['_PROP_' . $key] !== null) {
            if ($key === "manufacturer") {
                ...
                $name = $this->localizedIBlockProps[$key] ?? $key;
                
                $offer .= '<param name="' . $this->PrepareValue($name) . '" code="' . $key . '"' . (isset($arOffer['_PROP_' . $key . "_UNIT"]) ? ' unit="'
                        . $arOffer['_PROP_' . $key . "_UNIT"]
                        . '"' : "") . ">" . $this->PrepareValue($arOffer['_PROP_' . $key]) . "</param>\n";
            }
        }
        
        return [$offer, $arOffer];
    }
    
    /**
     * @param int $block_id
     */
    private function setSiteAddress(int $block_id): void
    {
    ...
    protected function PreWriteCatalog(): void
    {
     ...
     * @param $categories
     */
    protected function WriteCategories($categories): void
    {
        $stringCategories = "";
    ...
    protected function PreWriteOffers(): void
    {
        @fwrite($this->fp, "<offers>\n");
    }
    
    protected function PostWriteOffers(): void
    {
        @fwrite($this->fp, "</offers>\n");
    }
    
    protected function WriteOffers($offers): void
    {
        @fwrite($this->fp, $offers);
    }
    ...
    protected function PostWriteCatalog(): void
    {
        @fwrite($this->fp, "</shop>\n
            </yml_catalog>\n");
    }
    ...
    protected function GetCategories(): array
    {
        ...
        foreach ($this->iblocks as $id) {
            ...
            $dbRes         = CIBlockSection::GetList(["left_margin" => "asc"], $filter);
            $hasCategories = false;
    ...
    protected function BuildCategory($arCategory): string
    {
        $category =
            "<category id=\"" . $this->PrepareValue($arCategory["ID"]) . "\""
            . ((int)$arCategory["IBLOCK_SECTION_ID"] > 0 ?
    ...
    protected function BuildOffers(&$allCategories): void
    {
        ...
        foreach ($this->iblocks as $key => $id) {
            ...
            do {
                ...
                $elems            = [];
                $dbResProductsIds = CIBlockElement::GetList($order, $filter, false, $arNavStatParams, ['ID']);
    ...
    protected function processProductOffers(
        ...
        $iblockOffer,
        array $barcodes,
        array $highloadblockProductProps,
        array $highloadblockSkuProps,
        array $order,
        array $arFilter
    ): void {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($iblockOffer['IBLOCK_ID'])) {
                foreach ($product['offers'] as $offer) {
                    $offer['BARCODE']         = $barcodes[$offer['ID']] ?? '';
            ...
            if (!$existOffer) {
                $offer['BARCODE']          = $barcodes[$product["ID"]] ?? '';
     ...
     * @param $allCategories
     */
    protected function PutOffer($arOffer, $categories, $iblock, &$allCategories): void
    {
     ...
     * @return string
     */
    protected function BuildOffer($arOffer, $categories, $iblock, &$allCategories): string
    {
        $offer = "";
        ...
        if (strpos($arOffer['DETAIL_PAGE_URL'], "#SECTION_PATH#") !== false) {
            if (count($categories) !== 0) {
                ...
                if ((int)$category["IBLOCK_SECTION_ID"] !== 0) {
                    while (true) {
        ...
        foreach ($this->propertiesProduct[$iblock['ID']] as $key => $propProduct) {
            [$offer, $arOffer] = $this->addManufacturer($propProduct, $arOffer, $key, $offer);
        }
        ...
        if (isset($arOffer["MEASURE"]['SYMBOL_INTL'])) {
            if ($this->measure[$arOffer["MEASURE"]['SYMBOL_INTL']]) {
                ...
                $offer .= '<unit code="'
                    . $this->PrepareValue($arOffer["MEASURE"]['SYMBOL_INTL'])
                    . '" name="'
                    . $this->PrepareValue($arOffer["MEASURE"]['MEASURE_TITLE'])
                    . '" sym="'
                    . $this->PrepareValue($arOffer["MEASURE"]['SYMBOL_RUS'])
                    . '" />'
                    . "\n";
            }
        } else {
            try {
                $measure = ProductTable::getCurrentRatioWithMeasure($arOffer["ID"]);
            ...
            if ($this->measure[$measure[$arOffer["ID"]]["MEASURE"]['SYMBOL_INTL']]) {
                ...
                $offer .= '<unit code="'
                    . $this->PrepareValue($measure[$arOffer["ID"]]["MEASURE"]['SYMBOL_INTL'])
                    . '" name="'
                    . $this->PrepareValue($measure[$arOffer["ID"]]["MEASURE"]['MEASURE_TITLE'])
                    . '" sym="'
                    . $this->PrepareValue($measure[$arOffer["ID"]]["MEASURE"]['SYMBOL_RUS'])
                    . '" />'
                    . "\n";
            }
        }
    ...
    {
        try {
            $hlblockArr = HighloadBlockTable::getList([
     ...
     * @return array
     */
    private function getProductProperties(int $iblockId, array $highloadblockProductProps, array $product): array
    {
        // Get properties of product
        $resPropertiesProduct = [];
     ...
     * @return array
     */
    private function getProductCategories(array &$allCategories, int $iblockId, string $productId): array
    {
        $categories      = [];
        $dbResCategories = CIBlockElement::GetElementGroups($productId, true);
        
        while ($arResCategory = $dbResCategories->Fetch()) {
            $categories[$arResCategory["ID"]] = [
                'ID'   => $arResCategory["ID"],
    ...
    private function buildProductQuery($iblockId): array
    {
        $arSelect = [
            "ID",
            "LID",
     ...
     * @return string[]
     */
    private function buildOfferQuery($iblockId, $skuPropertyId): array
    {
        $arSelectOffer = [
            'ID',
            "NAME",
     ...
     * @return array
     */
    private function getAvailableHighloadProductProps($iblockId): array
    {
        $highloadblockProductProps = [];
        $productProps              = CIBlockproperty::GetList([], ["IBLOCK_ID" => $iblockId]);
     ...
     * @return array
     */
    private function getAvailableHighloadOfferSkuProps($iblockId): array
    {
        $highloadblockSkuProps = [];
        $skuProps              = CIBlockproperty::GetList([], ["IBLOCK_ID" => $iblockId]);
    ...
    private function getLocalizedIBlockProps(): array
    {
        return [
